    ...
    return (
        <div className="min-h-screen bg-[#FAFAF7] text-[#20223A] pb-12 pt-6 px-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-[#E2E4EB] shadow-xs">
                <div>
                    <h1 className="text-2xl font-extrabold flex items-center gap-2">
                        <Sprout className="w-6 h-6 text-emerald-600" /> Quản Lý Loại Cây & Sinh Trưởng Vườn (API Thật)
                    </h1>
                ...
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={fetchPlants} disabled={isLoading} className="font-bold flex items-center gap-1">
            ...
            {isLoading ? (
                ...
                <div className="p-12 text-center text-xs text-[#6B6D7A] bg-white rounded-2xl border border-[#E2E4EB]">
                        ...
                        <div key={plant.id} className="bg-white rounded-2xl p-6 border border-[#E2E4EB] shadow-xs space-y-4">
                            <div className="flex items-start justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <span className="text-[11px] font-bold text-[#6B6D7A] uppercase">{plant.code}</span>
                                ...
                                <div className="grid grid-cols-5 gap-1.5">
                                    {(plant.stages && plant.stages.length > 0
                                        ...
                                        <div key={idx} className="p-2 bg-emerald-50/50 border border-emerald-100 rounded-xl text-center space-y-1">
                                            ...
                                            <div className="text-[10px] font-bold text-emerald-900 truncate" title={stg.stage_name}>
                                                {stg.stage_name}
                                            </div>
                                        </div>
                                    ))}
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-2xl p-6 border border-[#E2E4EB] max-w-md w-full space-y-4 shadow-2xl">
                        <h3 className="text-lg font-bold flex items-center gap-2">
                            <Sprout className="w-5 h-5 text-emerald-600" /> Thêm loại cây mới
                        </h3>

                        <div className="space-y-3 text-xs">
                            <div>
                                <label className="font-bold block mb-1">Tên loại cây *</label>
                                <input
                                    type="text"
                                    value={plantName}
                            ...
                            <div>
                                <label className="font-bold block mb-1">Mã loại cây (Code) *</label>
                                <input
                                    type="text"
                                    value={plantCode}
                                    onChange={(e) => setPlantCode(e.target.value)}
                                    placeholder="CHERRY_BLOSSOM"
                                    className="w-full p-2.5 border border-[#E2E4EB] rounded-xl focus:ring-2 focus:ring-[#3C4097] outline-none uppercase font-mono"
                                />
                            </div>
                            <div>
                                <label className="font-bold block mb-1">Biểu tượng (Emoji / URL)</label>
                                <input
                                    type="text"
                                    value={iconUrl}
